...
use webrtc_gcc_apply::{InterArrival,TimestampGroup,TrendlineEstimator,AimdRateControl,RateControlInput,BitrateEstimator};
use std::{
    collections::HashMap,
...
static INTER_ARRIVAL_MANGER:Lazy<Mutex<InterArrival>> = Lazy::new(|| Mutex::new(InterArrival::new(60,0.001)));
static TRENDLINE_MANGER:Lazy<Mutex<TrendlineEstimator>> = Lazy::new(|| Mutex::new(TrendlineEstimator::new()));
static AIMD_MANGER:Lazy<Mutex<AimdRateControl>> = Lazy::new(|| Mutex::new(AimdRateControl::new(true)));
static RateControlInput_MANGER:Lazy<Mutex<RateControlInput>> = Lazy::new(|| Mutex::new(RateControlInput::new(webrtc_gcc_apply::BandwidthUsage::kBwNormal,Some((30*1024*1024) as f64))));
static BitrateEstimator_MANGER:Lazy<Mutex<BitrateEstimator>>=Lazy::new(||Mutex::new(BitrateEstimator::new()));
...
pub unsafe extern "C" fn HmdDriverFactory(
    ...
    extern "C" fn report_save(timestamp_ns: u64) {
        if let Some(stats) = &mut *STATISTICS_MANAGER.lock() {
            stats.report_save(Duration::from_nanos(timestamp_ns));
        }
    }
