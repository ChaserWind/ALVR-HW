    ...
    m_hEncoder(nullptr)
{
    ...
    m_hEncoder = hEncoder;
    OutputQP.open(get_path_head()+"QP.csv");
    OutputQP << "target_timestamp, QP_A, QP_B, QP_C, Target_Packet_Size, Real_Packet_Size" << std::endl;
}
...
bool NvEncoder::EncodeFrame(std::vector<std::vector<uint8_t>> &vPacket, NV_ENC_PIC_PARAMS *pPicParams, int states, uint64_t timestamp, float normalized_value, float TPS, bool ReEncoding)
{
    vPacket.clear();
    ...
    bool open = false;
    bool get_eframe = false;
    int count = get_frame_count();

    // Commment for not to capture
    if(get_eframe){
        ...
        if(count%(get_save_frame_feq()*2)>=(get_save_frame_feq()*2-10)||count%(get_save_frame_feq()*2)<=20){
            ReportSave(timestamp);
        }
    }

    NVENCSTATUS nvStatus = DoEncode(m_vMappedInputBuffers[bfrIdx], m_vBitstreamOutputBuffer[bfrIdx], pPicParams, states, timestamp, normalized_value);
    // if(save_frame){
    ...
    {
        m_iToSend++;
        SizeState = GetEncodedPacket(m_vBitstreamOutputBuffer, vPacket, true, save_frame, count, open, TPS, ReEncoding);
    }
    else
    {
        OutputQP << std::endl;
...
void NvEncoder::RunMotionEstimation(std::vector<uint8_t> &mvData)
{
    ...
    if (nvStatus == NV_ENC_SUCCESS)
    {
        ...
        bool SizeState = GetEncodedPacket(m_vMVDataOutputBuffer, vPacket, true, false, 100, false, 0.0, false);
        if (vPacket.size() != 1)
        {
...
float multiply_and_round(float value) {
    if(std::abs(value)<10){
        return std::round(std::abs(value) * 1.0f) / 10.0f;
    }
    else{
        return 1.0f;
    }
...
NVENCSTATUS NvEncoder::DoEncode(NV_ENC_INPUT_PTR inputBuffer, NV_ENC_OUTPUT_PTR outputBuffer, NV_ENC_PIC_PARAMS *pPicParams, int states, uint64_t timestamp, float normalized_value)
{
    ...
    if(true){
        ...
        picParams.qpDeltaMap = GetQpMapLevel(qpDeltaMapSize, states, multiply_and_round(normalized_value));
        picParams.qpDeltaMapSize = qpDeltaMapSize;
        OutputQP << timestamp << "," << Temp_A << "," << Temp_B << "," << Temp_C << ",";
    }
...
int8_t* NvEncoder::GetQpMapLevel(int QpMapSize, int states, float normalize_value){
    if(qp_map!=nullptr && states == 1 ){
        return qp_map;
    }
    else{
        int8_t lv_A = get_level(QP_A);
        int8_t lv_B = get_level(QP_B);
        ...
        if(states==2){
            ...
            if(lv_B<lv_C-1){
                Temp_B = Temp_B + (0.1+normalize_value);
            }
            else if(lv_A<lv_B-1){
                Temp_A = Temp_A + (0.1+normalize_value);
            }
            else if(QP_C < 24){
                Temp_C = Temp_C + (0.1+normalize_value);
            }
            else if(QP_B < 20){ //31
                Temp_C = 24.0;
                Temp_B = Temp_B + (0.1+normalize_value);
        ...
        if(QP_A != static_cast<int8_t>(std::ceil(Temp_A))){
            QP_A = static_cast<int8_t>(std::ceil(Temp_A));
            changing_MAP = true;
        }
        if (QP_B != static_cast<int8_t>(std::ceil(Temp_B))){
            QP_B = static_cast<int8_t>(std::ceil(Temp_B));
            changing_MAP = true;
        }
        if (QP_C !=  static_cast<int8_t>(std::ceil(Temp_C))){
            QP_C = static_cast<int8_t>(std::ceil(Temp_C));
            changing_MAP = true;
        }
        

        if(changing_MAP){
            int width = (GetEncodeWidth()+15)/16/2;
            int height = (GetEncodeHeight()+15)/16;
            int x = width*0.5;
            int y = height*0.5;
            qp_map = new int8_t[QpMapSize];
            int r1 = width*40/94; //30
            int r2 = width*15/94; //9
            for(int i = 0; i<width; i++){
                for(int j = 0; j<height; j++){
                    if(i>=x-r2 && i<=x+r2 && j>=y-r2 && j<=y+r2 &&(i-x)*(i-x)+(j-y)*(j-y)<=r2*r2){
                        qp_map[j*width*2+i] = QP_A;
                        qp_map[j*width*2+i+width] = QP_A;
                    }
                    else if (i>=x-r1 && i<=x+r1 && j>=y-r1 && j<=y+r1 && (i-x)*(i-x)+(j-y)*(j-y)<=r1*r1)
                    {
                        qp_map[j*width*2+i] = QP_B;
                        qp_map[j*width*2+i+width] = QP_B;
                    }
                    else{
                        qp_map[j*width*2+i] = QP_C;
                        qp_map[j*width*2+i+width] = QP_C;
                    }
                }
            }
        }
...
void NvEncoder::EndEncode(std::vector<std::vector<uint8_t>> &vPacket)
{
    ...
    bool SizeState = GetEncodedPacket(m_vBitstreamOutputBuffer, vPacket, false, false, 100, false, 0.0, false);
}

bool NvEncoder::GetEncodedPacket(std::vector<NV_ENC_OUTPUT_PTR> &vOutputBuffer, std::vector<std::vector<uint8_t>> &vPacket, bool bOutputDelay=false, bool saveFrame=false, int count=100, bool open=false, float TPS=0.0, bool ReEncoding = false)
{
    unsigned i = 0;
    ...
    for (; m_iGot < iEnd; m_iGot++)
    {
        ...
        OutputQP << TPS << ", " << lockBitstreamData.bitstreamSizeInBytes << ", " << saveFrame << ", " << ReEncode << std::endl;
        if(open && !ReEncode){
        ...
        if (m_vMappedInputBuffers[m_iGot % m_nEncoderBuffer]&&!ReEncode)
        {
